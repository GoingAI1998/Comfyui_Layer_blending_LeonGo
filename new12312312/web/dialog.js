...
class TransformBox {
    ...
    draw(ctx) {
        ...
        ctx.setLineDash([this.borderWidth * 3, this.borderWidth * 3]);
...
const dialog = {
    ...
    async save() {
        try {
            ...
            const response = await api.fetchApi('/canvas/save', {
                method: 'POST',
                headers: {
        ...
        api.addEventListener("show_canvas", async ({detail}) => {
            ...
            try {
                ...
                for (const node of res) {
                    if (node.type === "WebCanvas") {
                        console.log("Found WebCanvas node");
